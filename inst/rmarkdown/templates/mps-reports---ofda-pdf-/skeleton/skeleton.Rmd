...
params:
  ...
  docsubtitle: "MPS Theme Package Demo" # Subtitle for inclusion in final doc text
---
...
require(showtext)
showtext_auto()
showtext_opts(dpi = 300)      #'*Use this when knitting to pdf or html*
#showtext_opts(dpi = 96)       #'*Use this for inline viewing of plots*
require(ggplot2)
require(mpstheme)
require(scales)
...
title_ofda(title = params$doctitle)
```
``` {=latex}
% SUBTITLE CHUNK
\vspace{-30pt}
\fontspec{Poppins SemiBold}\begin{center}\large
\text{\color{gray}{`r paste0(params$docsubtitle)`}}\normalfont\end{center}
...
\text{\color{gray}{`r format(Sys.Date(), "%B %Y")`}}\normalfont
\end{minipage}
\normalfont
```
...
Suspendisse neque libero, tristique ut ante in, rhoncus aliquet lacus. Curabitur aliquam sapien diam, sit amet sollicitudin arcu faucibus ac. Nullam sed nisl tincidunt, sagittis lectus porta, aliquam arcu. In id imperdiet orci, ac facilisis est. In pretium pretium tortor, in commodo nisi vestibulum quis. Pellentesque eget gravida turpis. Integer sagittis, urna non mollis dapibus, erat felis vulputate ipsum, eu malesuada ligula augue nec nulla.
...
Use `\notebox{Some Note}`, `\warnbox{Some warning}`, `\impbox{Some important note}`, and `\specbox{Box Title}{Box Content}` for the special block notes and warnings seen below:
...
This already looks much more like an MPS-style, but it's still very much a basic `ggplot` look. So, let's change that by adding `theme_ofda()`. **NOTE** In the setup chunk of your document, you need to have `showtext_auto()` included after loading the `showtext` package. 
...
This is much better! So, what exactly is `theme_ofda()` doing to the base `ggplot()` theme? Well, see below for all the changes.
``` {r ggplot-theme-explain, echo = TRUE, eval = FALSE}
  theme(
    ...
    # Plot aesthetics
    plot.background = element_rect(color = pri.color, linewidth = 1),
